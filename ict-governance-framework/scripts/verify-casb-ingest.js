...
(async () => {
  ...
  const ingest = await axios.post('http://localhost:4000/api/assets/casb-ingest', SHADOW_PAYLOAD, { headers });
  console.log('INGEST', {
  ...
  const promote = await axios.post('http://localhost:4000/api/assets/validation/promote', {
